...
// Display buffer used for clearing the display panel
static uint8_t *p_display_buff_clear = NULL;

esp_err_t bsp_display_clear(esp_lcd_panel_handle_t disp_panel)
{
    if (p_display_buff_clear) {
        return esp_lcd_panel_draw_bitmap(disp_panel, 0, 0, BSP_LCD_H_RES, BSP_LCD_V_RES, p_display_buff_clear);
    }
...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    *ret_io = handles.io;
    ...
    uint8_t *display_buff = heap_caps_calloc(BSP_LCD_H_RES * BSP_LCD_V_RES * 2, sizeof(uint8_t), MALLOC_CAP_SPIRAM);
    if (display_buff == NULL) {
        ESP_LOGE(TAG, "Clear LCD panel: Display buffer is Null");
        // In case of failure here, we don't abort, it's not a critical necessity to use display clear functionality
    }
    p_display_buff_clear = display_buff;
